...
public class Case2_NuclearReactor {
    ...
    public static void runCase() {
        ...
        while(true) {
            ...
            final double alpha_EE = reactionGrowthFactor(rodPos_EE);
            final double rateNp1_EE = rateN_EE + timeStep * alpha_EE * rateN_EE;
            //Euler Implicit time-integration
            final double alpha_EI = reactionGrowthFactor(rodPos_EI);
            final double rateNp1_EI = rateN_EI + timeStep * alpha_EI * rateN_EI;
            // Runge-Kutta 2 Midpoint time-integration
            final double alpha_RK2 = reactionGrowthFactor(rodPos_RK2);
            final double htMid = 0.5 * timeStep;
            final double rateMid = rateN_RK2 + htMid * alpha_RK2 * rateN_RK2; //Useless in that particular case (RHS is not a function of position)
            final double rateNp1_RK2 = rateN_RK2 + timeStep * alpha_RK2 * rateMid;
